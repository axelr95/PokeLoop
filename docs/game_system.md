...
- **Coût** (v3) : `cout(tier) ≈ 10% du coût XP cumulé de l'équipe entière (6 membres)` pour atteindre ce niveau (section 3.3, courbe exponentielle), arrondi à un chiffre lisible — de 100 (tier I) à 1 870 000 (tier X). Basé sur l'équipe (pas un seul Pokémon) car c'est le pool de production réel du joueur. Recalculé après le passage à la courbe XP exponentielle (cf. `paliers_pokemon.md`). Uniforme pour tous les Pokémon, sera probablement différencié par lignée plus tard.
